...
- `reddit-watch` 初次 collect 与 diagnose 后 retry 均受 Reddit RSS HTTP 429 影响，仍保留 8 条 raw evidence；lane subagent 选出 3 条并标为 degraded。`github-ai-projects` 规范化为 derived partial，并从上游 GitHub evidence 生成 package。
- 14 条 reader lanes：12 ok、1 degraded（Reddit）、1 empty（OpenClaw）；每条均重新生成 `lane.md` 与 `lane-meta.json`，validation 通过。
- publish helper 成功创建 Feishu Docx，但 `FEISHU_HOME_CHANNEL` 未设置且 `im:message.send_as_user` OAuth scope 不可用；主 agent 对最终 `report.md` 的原文精选 payload preflight 后以 bot 身份发送至 Rook DM，live verification 确认 header 为 `Rook｜AI Agent 日报精选（2026-08-07）`。
- 最终文档已归档并推送至 knowledge-wiki commit `79da018`。

### 待验证 / 后续改动方向
- 为 publish helper 配置稳定 chat_id，并恢复 `lark-cli` user OAuth `im:message.send_as_user` scope，避免卡片必须以 bot identity 补发。
- 持续缓解 Reddit RSS 429；有 raw evidence 时保持由 lane agent 判断正文质量。
...
- 完成同日 deterministic collect、14 个 Hermes lane subagent、validation、assemble、Feishu Docx/精选卡片发布与 knowledge-wiki 归档。
- collect preflight 使用 repo-local `uvx --from /Users/haha/workspace/signals-engine signals-engine`、生产配置 `/Users/haha/.signal-engine/config/lanes.yaml`、data-dir `/Users/haha/.daily-lane-data`；registry 覆盖 weather / Reddit / HN / Claude / Codex / OpenClaw / Polymarket。
- `reddit-watch` 首次 collect 及 diagnose 后 retry 均遇 RSS HTTP 429；首次仍保存 8 条 raw evidence，lane agent 选出 2 条并标为 degraded。`github-ai-projects` 已规范化为 derived partial；`openclaw-watch` 成功但标准 package mapping 缺失，已从 raw evidence 补建 package。
- 14 条 reader lanes：12 ok、1 degraded（Reddit）、1 blocked（Claude Code 无原始可核验新增）；所有 lane.md 与 lane-meta.json 已生成，validation 与 section-heading cross-check 通过，lane agents 共选中 49 条。
- publish helper 创建 Feishu Docx，但因 `FEISHU_HOME_CHANNEL` 未设置未自动发卡；主 agent 对最终 report.md 原文精选卡 payload 做 preflight 后，以 bot 身份发送至 Rook DM，live verification 确认标题为 `Rook｜AI Agent 日报精选（2026-08-02）`。

### 待验证 / 后续改动方向
- 为 publish helper 提供明确 chat_id，避免 `FEISHU_HOME_CHANNEL` 缺失时需要主 agent 补发卡片。
- 修复 runtime config / package mapping 对 `openclaw-watch` → `openclaw` 的正式映射。
- 继续缓解 Reddit RSS 429；有 raw evidence 时保持由 lane agent 判断正文质量。
...
- 完成同日 deterministic collect、14 个 Hermes lane subagent、validation、assemble、Feishu Docx/精选卡片发布与 knowledge-wiki 归档。
- collect preflight 使用 repo-local `uvx --from /Users/haha/workspace/signals-engine signals-engine`、生产配置 `/Users/haha/.signal-engine/config/lanes.yaml`、data-dir `/Users/haha/.daily-lane-data`；registry 覆盖 weather / Reddit / HN / Claude / Codex / OpenClaw / Polymarket。
- `reddit-watch` 的首次 collect、diagnose 后 retry 均因 RSS HTTP 429 非零，但保留 8 条 raw evidence；lane agent 选出 2 条并标为 degraded。`github-ai-projects` 已按 derived lane 规范化为 `partial`，并由上游 GitHub evidence 生成 package。
- 14 条 reader lanes：11 ok、1 degraded（Reddit RSS 429）、2 empty（Claude Code、OpenClaw）；全部 `lane.md` + `lane-meta.json` 已生成，validation 与 section-heading cross-check 通过，lane agents 共选中 48 条。
- publish helper 创建 Feishu Docx，但缺少 `FEISHU_HOME_CHANNEL` 未自动发卡；主 agent 对最终 `report.md` 抽取的 card payload 预检通过后，以 bot 身份发送至 Rook DM，live verification 确认标题为 `Rook｜AI Agent 日报精选（2026-08-01）`。
...
- 完成同日 deterministic collect、14 个 Hermes lane subagent、validation、assemble、Feishu Docx/精选卡片发布与 knowledge-wiki 归档。
- collect preflight 使用 repo-local `uvx --from /Users/haha/workspace/signals-engine signals-engine`、生产配置 `/Users/haha/.signal-engine/config/lanes.yaml`、data-dir `/Users/haha/.daily-lane-data`；registry 覆盖 weather / Reddit / HN / Claude / Codex / OpenClaw / Polymarket。
- `reddit-watch` 的 collect 与同 run diagnose/retry 均因 RSS HTTP 429 非零，但留下 8 条 raw evidence；lane agent 仅选出 1 条并标为 degraded。`github-ai-projects` 维持 derived collect artifact；OpenClaw 标准 package mapping 仍缺失，已由成功的 `openclaw-watch` raw evidence 补建 package 后交给 lane agent。
- 14 条 reader lanes：12 ok、1 degraded（Reddit RSS 429）、1 empty（Claude Code）；全部 `lane.md` + `lane-meta.json` 已生成，validation 与 section-heading cross-check 通过，lane agents 共选中 50 条。
- publish helper 创建 Feishu Docx，但未设置 `FEISHU_HOME_CHANNEL` 而未自动发卡；主 agent 对由最终 `report.md` 抽取的 card payload 做 preflight 后，以 bot 身份发送至 Rook DM，live verification 确认标题为 `Rook｜AI Agent 日报精选（2026-07-30）`，并撤回同 run 的最小降级卡片。
- 最终文档已归档并推送至 knowledge-wiki commit `77f2d07`。
...
- 完成同日 deterministic collect、14 个 Hermes lane subagent、validation、assemble、Feishu Docx/精选卡片发布与 knowledge-wiki 归档。
- collect preflight 使用 repo-local `uvx --from /Users/haha/workspace/signals-engine signals-engine`、生产配置 `/Users/haha/.signal-engine/config/lanes.yaml`、data-dir `/Users/haha/.daily-lane-data`；registry 覆盖 weather / Reddit / HN / Claude / Codex / OpenClaw / Polymarket。
- `reddit-watch` 在 RSS HTTP 429 后重试，保留 16 条 raw evidence；lane agent 选出 3 条。`github-ai-projects` 维持 derived collect artifact；OpenClaw 标准 package 映射仍缺失，直接以 `openclaw-watch` raw evidence 交给 lane agent，最终标为 degraded。
- 14 条 reader lanes：12 ok、1 degraded（OpenClaw package mapping gap）、1 empty（Claude Code）；全部 `lane.md` + `lane-meta.json` 已生成，validation 与 section-heading cross-check 通过，lane agents 共选中 51 条。
- publish helper 创建 Feishu Docx，但因未设置 `FEISHU_HOME_CHANNEL` 未自动发卡；主 agent 将最终 `report.md` 抽取的 card payload 预检后以 bot 身份发送至 Rook DM，live verification 确认 header 为 `Rook｜AI Agent 日报精选（2026-07-29）`。
...
### 运行记录
- 完成同日 deterministic collect、14 个 Hermes lane subagent、validation、assemble、Feishu Docx 与精选卡片发布，以及 knowledge-wiki 归档。
- collect preflight 使用 repo-local `uvx --from /Users/haha/workspace/signals-engine signals-engine`、生产配置 `/Users/haha/.signal-engine/config/lanes.yaml`、data-dir `/Users/haha/.daily-lane-data`；完整 registry 覆盖 weather / Reddit / HN / Claude / Codex / OpenClaw / Polymarket。
- `reddit-watch` 首次和 retry 均受 Reddit RSS HTTP 429 影响，但留下 16 条 raw evidence；lane subagent 从中选出可核验内容。`github-ai-projects` 维持 derived collect artifact；OpenClaw collector 正常结束但没有 raw 信号，lane subagent 标为 empty。
- 14 条 reader lanes：13 ok、1 empty（OpenClaw）、0 degraded/blocked；全部 `lane.md` + `lane-meta.json` 已生成，validation 与 section-heading cross-check 通过，lane subagents 共选中 47 条。
- publish helper 创建 Feishu Docx，但缺 `FEISHU_HOME_CHANNEL` 未能自动发卡；主 agent 对从最终 report.md 抽取的 card payload 做 preflight 后补发至 Rook DM，并 live verify 标题为 `Rook｜AI Agent 日报精选（2026-07-27）`。

### 待验证 / 后续改动方向
- 修复 publish helper 对明确 chat_id 的支持，避免 `FEISHU_HOME_CHANNEL` 缺失时需要主 agent 补发卡片。
- 修复 runtime config / package mapping 对 `openclaw-watch` → `openclaw` 的正式映射。
- 继续缓解 Reddit RSS 429；在保留 raw evidence 时由 lane subagent 判断正文质量。
...
- 完成同日 deterministic collect、14 个 Hermes lane subagent、validation、assemble、Feishu Docx/精选卡片发布与 knowledge-wiki 归档。
- collect preflight 使用 repo-local `uvx --from /Users/haha/workspace/signals-engine signals-engine`、生产配置 `/Users/haha/.signal-engine/config/lanes.yaml`、data-dir `/Users/haha/.daily-lane-data`；registry 覆盖 weather / Reddit / HN / Claude / Codex / OpenClaw / Polymarket。
- `reddit-watch` 首次与 retry 均因 RSS HTTP 429 失败，但保留 raw evidence；lane subagent 将 Reddit 标为 degraded 并只选取可核验内容。`github-ai-projects` 作为 derived lane 被规范化为 partial collect；OpenClaw 的标准 package 映射仍缺失，主 agent 由成功的 `openclaw-watch` raw evidence 补建 package 后交给 lane subagent。
- 14 条 reader lanes：12 ok、1 degraded（Reddit）、1 empty（Claude Code）；lane subagents 共选中 52 条，validation 与 report heading cross-check 通过。
- publish helper 创建 Feishu Docx，但未设置 `FEISHU_HOME_CHANNEL` 而未发送卡片；主 agent 以 preflight 通过的原文精选 payload 发送至 Rook DM，live verification 确认 header 为 `Rook｜AI Agent 日报精选（2026-07-24）`。最终文档归档至 knowledge-wiki commit `7cdd49b`。

### 待验证 / 后续改动方向
- 为 publish helper 显式注入稳定 chat_id，避免 `FEISHU_HOME_CHANNEL` 缺失时需要补发卡片。
- 修复 runtime config / package mapping 对 `openclaw-watch` → `openclaw` 的正式映射。
- 继续缓解 Reddit RSS 429；有 raw evidence 时保持由 lane agent 决定 degraded/empty/ok。
...
### 运行记录
- 06:00 master cron 走 Hermes 原生 subagent lane 架构完成日报：collect / package / delegate / validate / assemble / Feishu publish / knowledge-wiki archive。
- collect preflight 使用 repo-local `uvx --from /Users/haha/workspace/signals-engine signals-engine`、生产配置 `/Users/haha/.signal-engine/config/lanes.yaml`、data-dir `/Users/haha/.daily-lane-data`；lane registry 覆盖 weather / reddit / HN / Claude / Codex / OpenClaw / Polymarket。
- Reddit collect 首次和 retry 均失败且 raw_file_count=0，本轮 Reddit lane 由 subagent 标记 degraded；OpenClaw package 仍因 runtime config 缺口未生成，但已由 subagent 直接从 `openclaw-watch` raw evidence 产出 lane。
- publish helper 首发 Feishu card header 仍缺少 `Rook｜`，同 run 已重发 corrected card、live verify，并撤回 superseded card；最终 publish-state 指向 corrected message。

### 待验证 / 后续改动方向
- 修复 runtime config / prepare package 对 `openclaw` 的正式映射，避免每次靠主 agent 从 raw evidence 兜底。
- 排查 `reddit-watch` collect 失败原因；若 Reddit 仍在 reader-facing order，应该让 collect 至少产出可审计 raw evidence，或者明确产品上移除。
- 修复 `publish_report.py` helper，使 `--title Rook｜...` 真正写入 interactive card header，避免每次发布后手动 corrected resend。
