...
def test_build_lark_create_command_uses_markdown_file():
    ...
    assert cmd[:4] == ['lark-cli','docs','+create','--as']; assert 'user' in cmd; assert '--title' in cmd; assert 'AI Agent 日报（2026-04-26）skill-preview' in cmd; assert '--markdown' in cmd; assert '@/tmp/report.md' in cmd
