...
def _recent_report_sources(report_date: str, history_root: Path) -> list[tuple[str, Path]]:
    current=date.fromisoformat(report_date)
    reports=[]
    for days_back in (1,2):
        recent_date=(current-timedelta(days=days_back)).isoformat()
        report=history_root/recent_date/'report.md'
        if report.exists():
            reports.append((recent_date,report))
    return reports
...
def prepare_lane_packages(report_date: str, signal_root: Path, runtime_root: Path, selected_items_path: Path|None=None) -> dict[str, Path]:
    ...
    for lane in LANES:
        ...
        recent_report_paths=[]
        for recent_date, report in recent_reports:
        ...
        context={'report_date':report_date,'lane':lane,'signal_lane':SIGNAL_LANE_MAP[lane],'skill':LANE_SKILL_MAP[lane],'raw_dir':str(raw),'raw_corpus_status':status,'raw_file_count':len(files),'output_markdown':str(output/'lane.md'),'output_meta':str(output/'lane-meta.json'),'selected_items_path':str(selected_items_path) if selected_items_path else None,'selected_items_mode':'audit_only','recent_report_paths':[str(p) for p in recent_report_paths],'recent_report_mode':'agent_dedup_reference_only'}
